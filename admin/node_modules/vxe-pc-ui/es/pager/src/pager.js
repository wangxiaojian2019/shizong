...
export default {
    ...
    computed: Object.assign(Object.assign({}, {}), { computePageCount() {
            const $xePager = this;
            const props = $xePager;
    ...
    methods: {
        ...
        jumpPageEvent(evnt, currentPage) {
            const $xePager = this;
            const props = $xePager;
        ...
        changeCurrentPage(currentPage, evnt) {
            const $xePager = this;
            const props = $xePager;
            $xePager.$emit('update:currentPage', currentPage);
            if (evnt && currentPage !== props.currentPage) {
        ...
        renderNumber(h, showJump) {
            ...
            if (showJump && isLt) {
                ...
                nums.push(h('button', {
                    ...
                    on: {
                        click: (evnt) => $xePager.jumpPageEvent(evnt, 1)
                    }
            ...
            numList.forEach((item, index) => {
                ...
                if (number <= pageCount) {
                    ...
                    nums.push(h('button', {
                        ...
                        on: {
                            click: (evnt) => $xePager.jumpPageEvent(evnt, number)
                        }
                    }, `${number}`));
                }
            });
            if (showJump && isGt) {
                ...
                nums.push($xePager.renderNextJump(h, 'button'), h('button', {
                    ...
                    on: {
                        click: (evnt) => $xePager.jumpPageEvent(evnt, pageCount)
                    }
                }, pageCount));
            }
            if (numberSlot) {
