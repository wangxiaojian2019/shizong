...
function hangleStorageDefaultValue(value, isAll) {
  return _xeUtils.default.isBoolean(value) ? value : isAll;
}
...
function restoreCustomStorage($xeTable) {
  ...
  if (storage && (customConfig ? (0, _utils.isEnableConf)(customOpts) : customOpts.enabled) && (isCustomResizable || isCustomVisible || isCustomFixed || isCustomSort || isCustomAggFunc)) {
    if (!tableId) {
...
function handleUpdateRowGroup($xeTable, groupFields) {
  ...
  const aggConfs = [];
  if (groupFields) {
    (_xeUtils.default.isArray(groupFields) ? groupFields : [groupFields]).forEach(field => {
...
const Methods = {
  ...
  restoreCustomStorage() {
    ...
    const isCustomFixed = hangleStorageDefaultValue(storageOpts.fixed, isAllCustom);
    const isCustomSort = hangleStorageDefaultValue(storageOpts.sort, isAllCustom);
    if (storage && (customConfig ? (0, _utils.isEnableConf)(customOpts) : customOpts.enabled) && (isCustomResizable || isCustomVisible || isCustomFixed || isCustomSort)) {
      if (!tableId) {
        (0, _log.errLog)('vxe.error.reqProp', ['id']);
        return;
      }
          ...
          if (animation) {
            ...
            if (firstRowRest) {
              ...
              if (dragRangeList.length) {
                const dtTrList = el.querySelectorAll(dragRangeList.map(row => `.vxe-body--row[rowid="${(0, _util.getRowid)($xeTable, row)}"]`).join(','));
                (0, _anime.moveRowAnimateToTb)(dtTrList, offsetRate * dragRowHeight);
              }
            }
            const newTrList = el.querySelectorAll(`.vxe-body--row[rowid="${dragRowid}"]`);
            const newTrEl = newTrList[0];
            if (dragOffsetTop > -1 && newTrEl) {
              (0, _anime.moveRowAnimateToTb)(newTrList, dragOffsetTop - newTrEl.offsetTop);
            }
          }
          updateRowOffsetTop($xeTable);
          updateRowExpandStyle($xeTable);
        ...
        return $xeTable.$nextTick().then(() => {
          ...
          if (animation) {
            ...
            if (dragColWidth && dragAllTargetCols.length) {
              const _newColIndex = _xeUtils.default.findIndexOf(visibleColumn, column => !!dragNewColumn && column.id === dragNewColumn.id);
              ...
              if (firstColRest) {
                ...
                _xeUtils.default.eachTree([newColumn], column => {
                ...
                if (dragRangeList.length) {
                  ...
                  (0, _anime.moveColAnimateToLr)(dtTrList, offsetRate * dragColWidth);
                }
              }
              ...
              if (dragOffsetLeft > -1 && newTdEl) {
                (0, _anime.moveColAnimateToLr)(newTrList, dragOffsetLeft - newTdEl.offsetLeft);
              }
            }
          }
          updateColumnOffsetLeft($xeTable);
  ...
  setRowGroups(fieldOrColumns) {
    ...
    const confList = fieldOrColumns ? _xeUtils.default.isArray(fieldOrColumns) ? fieldOrColumns : [fieldOrColumns] : [];
  ...
  setMergeCells(merges) {
    ...
    return $xeTable.$nextTick().then(() => {
      ...
      if (expandColumn && mergeBodyList.length) {
        (0, _log.warnLog)('vxe.error.errConflicts', ['type=expand', 'merge-cells | span-method']);
      }
      $xeTable.updateCellAreas();
      return updateStyle($xeTable);
    });
