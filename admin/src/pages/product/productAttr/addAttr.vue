<template>
	<el-dialog :visible.sync="modal" @closed="onCancel" title="商品规格" width="1000px" v-loading="spinShow">
				...
				<el-col :span="23" class="noForm" :key="index">
					<el-form-item label="">
						<div class="specifications">
							<draggable group="specifications" :list="formDynamic.spec" handle=".move-icon"
								animation="300">
								<div class="specifications-item active" v-for="(item, index) in formDynamic.spec"
									:key="index">
									<div class="move-icon">
									...
									<div class="specifications-item-box">
										...
										<div class="rulesBox ml30">
											<draggable class="item" :list="item.detail" handle=".drag">
												<div v-for="(j, indexn) in item.detail" :key="indexn"
													class="mr10 spec drag">
													<i class="el-icon-error"
														@click="handleRemove2(item.detail, indexn)"></i>

													<el-input style="width: 120px" v-model="item.detail[indexn]"
														placeholder="规格值" maxlength="30">
												...
												<el-popover :ref="'popoverRef_' + index" placement="" width="210"
													...
													<el-input :ref="'inputRef_' + index" placeholder="请输入规格值"
														...
														@blur="createAttr(item.detail.attrsVal, index)" maxlength="30"
														show-word-limit>
													</el-input>
													<div class="addfont" slot="reference" type="text" v-db-click>添加规格值
													</div>
												</el-popover>
											</draggable>
										</div>
							...
							<el-button v-if="formDynamic.spec.length < 4" v-db-click
								@click="handleAddRole()">添加新规格</el-button>
						</div>
					</el-form-item>
				</el-col>
			</el-row>
		...
		<span slot="footer" class="dialog-footer">
			...
			<el-button type="primary" :loading="modal_loading" v-db-click
				@click="handleSubmit('formDynamic')">确定</el-button>
		</span>
	</el-dialog>
</template>
