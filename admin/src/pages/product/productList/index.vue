	...
	<div class="article-manager">
		<el-card :bordered="false" shadow="never" class="ivu-mt" :body-style="{ padding: 0 }">
			<div class="padding-add">
				<el-form ref="artFrom" :model="artFrom" label-width="80px" label-position="right" inline
					@submit.native.prevent>
					<div class="acea-row search-form">
						<div class="search-form-box">
							<el-form-item label="商品搜索：" label-for="store_name">
								<el-input clearable placeholder="请输入商品名称/关键字/ID" v-model="artFrom.store_name"
									class="form_content_width" />
							</el-form-item>
							<el-form-item label="商品类型：">
								<el-select v-model="artFrom.virtual_type" clearable placeholder="全部"
									class="form_content_width">
							...
							<el-form-item label="配送方式：">
								<el-select v-model="artFrom.logistics" clearable placeholder="全部"
									class="form_content_width">
									<el-option label="全部" value="" />
							...
							<template v-if="collapse">
								<el-form-item label="商品标签：" label-for="store_name">
									<div class="labelInput acea-row row-between-wrapper form_content_width"
										@click="openStoreLabel">
										<div style="width: 90%">
											<div v-if="storeLabelList.length">
												<el-tag class="mr5" closable v-for="(item, index) in storeLabelList"
													:key="index"
													@close="closeStoreLabel(item)">{{ item.label_name }}</el-tag>
											</div>
								...
								<el-form-item label="商品规格：">
									<el-select v-model="artFrom.spec_type" clearable placeholder="全部"
										class="form_content_width">
										<el-option label="全部" value="" />
								...
								<el-form-item label="会员专属：">
									<el-select v-model="artFrom.vip_product" clearable placeholder="全部"
										class="form_content_width">
										<el-option label="全部" value="" />
								...
								<el-form-item label="是否礼物：">
									<el-select v-model="artFrom.is_gift" clearable placeholder="全部"
										class="form_content_width">
										<el-option label="全部" value="" />
										<el-option label="否" value="0" />
										<el-option label="是" value="1" />
								...
								<el-form-item label="添加时间：">
									<el-date-picker class="form_range_content_width" clearable v-model="timeVal"
										type="daterange" :editable="false" @change="onchangeTime" format="yyyy/MM/dd"
										value-format="yyyy/MM/dd" start-placeholder="开始日期" end-placeholder="结束日期"
										:picker-options="pickerOptions" style="width: 250px"></el-date-picker>
								</el-form-item>
								<el-form-item label="库存：" label-for="store_name">
									<el-input clearable placeholder="最小值" v-model="artFrom.stock_s[0]"
										class="form_range_content_width" />
									~
									<el-input clearable placeholder="最大值" v-model="artFrom.stock_s[1]"
										class="form_range_content_width" />
								</el-form-item>
								<el-form-item label="价格：" label-for="store_name">
									<el-input clearable placeholder="最小值" v-model="artFrom.price_s[0]"
										class="form_range_content_width" />
									~
									<el-input clearable placeholder="最大值" v-model="artFrom.price_s[1]"
										class="form_range_content_width" />
								</el-form-item>
								<el-form-item label="销量：" label-for="store_name">
									<el-input clearable placeholder="最小值" v-model="artFrom.sales_s[0]"
										class="form_range_content_width" />
									~
									<el-input clearable placeholder="最大值" v-model="artFrom.sales_s[1]"
										class="form_range_content_width" />
								</el-form-item>
							</template>
						</div>
		...
		<el-card :bordered="false" shadow="never" class="ivu-mt mt16" :body-style="{ padding: '0 20px 20px' }">
			...
			<div class="Button">
				...
				<el-button v-auth="['product-crawl-save']" type="success" class="mr14" v-db-click
					@click="onCopy">商品采集</el-button>
				<el-dropdown class="bnt mr14" @command="batchSelect">
					...
					<el-dropdown-menu slot="dropdown">
						...
						<el-dropdown-item v-auth="['product-product-product_show']" v-if="artFrom.type === '1'"
							:command="7">批量下架</el-dropdown-item>
						<el-dropdown-item v-auth="['product-product-product_show']" v-if="artFrom.type === '2'"
							:command="8">批量上架</el-dropdown-item>
						<el-dropdown-item v-auth="['product-product-product_show']"
							:command="9">设置商品标签</el-dropdown-item>
						<el-dropdown-item v-auth="['product-product-product_show']"
							:command="10">是否支持送礼</el-dropdown-item>
						<el-dropdown-item v-auth="['product-product-product_show']" v-if="artFrom.type !== '6'"
							:command="11">移到回收站</el-dropdown-item>
						<el-dropdown-item v-auth="['product-product-product_show']" v-if="artFrom.type == '6'"
							:command="12">恢复商品</el-dropdown-item>
					</el-dropdown-menu>
				</el-dropdown>
				...
				<el-button v-auth="['export-storeProduct']" class="export" v-db-click
					@click="onExports(0)">数据导出</el-button>
			</div>
            ...
            >
              秒杀
				...
				<el-table-column label="用户信息" min-width="100">
					<template slot-scope="scope">
					<span>{{ scope.row.uid }}<span  v-if="scope.row.plate.number_plate"> | {{scope.row.plate.number_plate}} </span></span>
					</template>
				</el-table-column>
				...
				<el-table-column label="状态" min-width="100">
					<template slot-scope="scope">
						<el-switch class="defineSwitch" :active-value="1" :inactive-value="0"
							v-model="scope.row.is_show" :value="scope.row.is_show"
							:disabled="scope.row.stop_status ? true : false" @change="changeSwitch(scope.row)"
							size="large" active-text="上架" inactive-text="下架">
						</el-switch>
					</template>
				</el-table-column>
            ...
            <el-divider direction="vertical"></el-divider> -->
						...
						<el-dropdown size="small">
							...
							<el-dropdown-menu slot="dropdown">
								...
								<el-dropdown-item v-db-click
									@click.native="openModal(scope.row, 'vipPriceSet')">会员价管理</el-dropdown-item>
								<el-dropdown-item v-db-click
									@click.native="openModal(scope.row, 'brokerageSet')">佣金管理</el-dropdown-item>
								<el-dropdown-item v-if="artFrom.type === '6'" v-db-click
									@click.native="del(scope.row, '恢复商品', scope.$index)">恢复商品</el-dropdown-item>
								<el-dropdown-item v-if="artFrom.type === '6'" v-db-click
									@click.native="fullDel(scope.row, '彻底删除', scope.$index)">彻底删除</el-dropdown-item>
								<el-dropdown-item v-else v-db-click
									@click.native="del(scope.row, '移入回收站', scope.$index)">移到回收站</el-dropdown-item>
							</el-dropdown-menu>
						</el-dropdown>
					</template>
				</el-table-column>
		...
		<!-- 生成淘宝京东表单-->
		<el-dialog :visible.sync="modals" class="Box" title="复制淘宝、天猫、京东、苏宁、1688" :close-on-click-modal="false"
			width="720px">
			<tao-bao ref="taobaos" v-if="modals" @on-close="onClose"></tao-bao>
		</el-dialog>
		<el-dialog :visible.sync="batchModal" class="batch-box" title="批量设置" :show-close="true"
			:close-on-click-modal="false" width="540px">
			<el-form class="batchFormData" ref="batchFormData" :rules="ruleBatch" :model="batchFormData"
				label-width="90px" label-position="right" @submit.native.prevent>
				<el-row :gutter="24">
                  ...
                  item.html + item.cate_name
					...
					<el-col :span="24" v-if="batchType == 2">
						...
						<el-form-item label="" v-if="batchFormData.freight == 2">
							<div class="acea-row">
								<el-input-number :controls="false" :min="0" v-model="batchFormData.postage"
									placeholder="请输入金额" class="perW20 maxW" />
							</div>
						</el-form-item>
						<el-form-item label="" v-if="batchFormData.freight == 3" prop="temp_id">
							<div class="acea-row">
								<el-select v-model="batchFormData.temp_id" clearable placeholder="请选择运费模板"
									style="width: 414px">
					...
					<el-col :span="24" v-if="[3, 4, 5, 6].includes(batchType)">
						...
						<el-form-item label="赠送积分：" prop="give_integral" v-if="batchType == 3">
							<el-input-number :controls="false" v-model="batchFormData.give_integral" :min="0"
								:max="9999999999" placeholder="请输入积分" style="width: 100%" />
						</el-form-item>
						<el-form-item label="赠送优惠券：" v-if="batchType == 4">
							<div v-if="couponName.length" class="mb20">
								<el-tag closable v-for="(item, index) in couponName" :key="index"
                  ...
                  item.title
								...
								<div class="labelInput acea-row row-between-wrapper" v-db-click @click="openLabel">
									<div style="width: auto">
										<div v-if="dataLabel.length">
											<el-tag class="m-r-2" closable v-for="(item, index) in dataLabel"
												@close="closeLabel(item)" :key="index">{{ item.label_name }}</el-tag>
										</div>
					...
					<el-col :span="24" v-if="batchType == 10">
						<el-form-item label="开启送礼：">
							<el-switch v-model="batchFormData.is_gift" class="defineSwitch" active-text="开启"
								inactive-text="关闭" :active-value="1" :inactive-value="0" size="large">
							</el-switch>
							<div class="tips-info">开启送礼后，移动端商品详情的底部菜单显示送礼按钮</div>
						</el-form-item>
						<el-form-item v-if="batchFormData.is_gift" label="礼品附加费：">
							<el-input-number :controls="false" :min="0" :max="100000" v-model="batchFormData.gift_price"
								placeholder="礼品附加费" class="input-number-unit-class" class-unit="元" />
		...
		<el-dialog :visible.sync="tagShow" title="请选择商品标签" :show-close="true" width="540px"
			...
			<goodsLabel ref="goodsLabel" :defaultLabelList="goodsLabelList" @activeLabel="activeGoodsLabel"
				@close="labelClose"></goodsLabel>
		</el-dialog>
		<!-- 用户标签 -->
		...
		<!-- 商品导入 -->
		<el-dialog :visible.sync="importShow" title="商品导入" width="900px" :show-close="true"
			:close-on-click-modal="false">
		...
		<el-dialog :visible.sync="storeLabelShow" title="选择商品标签" width="540">
			<storeLabelList v-if="storeLabelShow" ref="storeLabel" @activeData="activeStoreData"
				@close="storeLabelClose"></storeLabelList>
		</el-dialog>
	</div>
</template>
