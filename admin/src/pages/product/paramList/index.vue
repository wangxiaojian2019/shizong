			...
			<div class="padding-add">
				<el-form ref="artFrom" :model="artFrom" label-width="80px" label-position="right" class="tabform"
					...
					<el-form-item label="模板搜索：">
						<el-input clearable v-model="artFrom.name" placeholder="请输入专场id"
		...
		<el-card :bordered="false" shadow="never" class="ivu-mt mt16">
			...
			<el-table ref="table" :data="tableList" v-loading="loading" highlight-current-row :row-key="getRowKey"
				...
				<el-table-column label="用户信息" min-width="130">
					<template slot-scope="scope">
						<span>{{ scope.row.uid }} | {{ scope.row.name }}</span>
					</template>
				</el-table-column>
				...
				<el-table-column label="专场信息" min-width="130">
					<template slot-scope="scope">
						<span>{{ scope.row.special }}</span>
					</template>
				</el-table-column>
				<el-table-column label="代理价格(元)" min-width="130">
					<template slot-scope="scope">
						<span>{{ scope.row.agent_price }}</span>
					</template>
				</el-table-column>
