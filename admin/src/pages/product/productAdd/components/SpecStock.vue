	...
	<el-row :gutter="24">
		...
		<el-col :span="24" v-if="formValidate.spec_type === 1" class="noForm">
			<el-form-item label="商品规格：" prop="">
				<div class="specifications">
					<draggable group="specifications" :disabled="attrs.length < 2" :list="attrs" handle=".move-icon"
						@end="onMoveSpec" animation="300">
						<div class="specifications-item active" v-for="(item, index) in attrs" :key="index"
							...
							<div class="specifications-item-box">
								...
								<div class="specifications-item-name mb18">
									<el-input v-model="item.value" placeholder="规格名称"
										@change="attrChangeValue(index, item.value)" @focus="handleFocus(item.value)"
										class="specifications-item-name-input" maxlength="30"
										show-word-limit></el-input>
									<el-checkbox class="ml20" v-model="item.add_pic"
										:disabled="!item.add_pic && !canSel" :true-label="1" :false-label="0"
										@change="(e) => addPic(e, index)">添加规格图</el-checkbox>
									<el-tooltip class="item" effect="dark" content="添加规格图片, 仅支持打开一个(建议尺寸:800*800)"
										placement="right">
										<i class="el-icon-info"></i>
									</el-tooltip>
								</div>
								<div class="rulesBox ml30">
									<draggable class="item" :list="item.detail" :disabled="item.detail.length < 2"
										handle=".drag" @end="onMoveSpec">
										<div v-for="(det, indexn) in item.detail" :key="indexn" class="mr10 spec drag">
											<i class="el-icon-error"
												@click="handleRemove2(item.detail, indexn, det.value)"></i>

											<el-input style="width: 120px" v-model="det.value" placeholder="规格值"
												@change="attrDetailChangeValue(det.value, index)"
												@focus="handleFocus(det.value)" maxlength="30" @blur="handleBlur()">
											...
											<div class="img-popover" v-if="item.add_pic">
												...
												<i v-if="det.pic" class="img-del el-icon-error"
													@click="handleRemoveImg(det)"></i>
											</div>
										</div>
										<el-popover :ref="'popoverRef_' + index" placement="" width="210"
											trigger="click" @after-enter="handleShowPop(index)">
											<el-input :ref="'inputRef_' + index" placeholder="请输入规格值"
												v-model="formDynamic.attrsVal"
												@keyup.enter.native="createAttr(formDynamic.attrsVal, index)"
												@blur="createAttr(formDynamic.attrsVal, index)" maxlength="30"
												show-word-limit>
											</el-input>
		...
		<el-col :span="24" v-if="formValidate.spec_type === 1">
			<el-form-item label="商品属性：" class="labeltop" v-if="manyFormValidate.length">
				<VirtualScroll :data="manyFormValidate" :buffer="50" :height="62" key-prop="index"
					@change="(renderData) => (virtualList = renderData)">
					<!-- <el-table row-key="id" :data="virtualList" height="500px"> </el-table> -->
					<el-table row-key="index" height="700px" :data="manyFormValidate" style="width: 100%"
						:cell-class-name="tableCellClassName" :span-method="objectSpanMethod" border :key="tableKey">
							...
							<template slot-scope="scope">
								...
								<template v-if="scope.$index == 0">
									<template v-if="item.key">
										<div
											v-if="attrs.length && attrs[scope.column.index] && manyFormValidate.length">
											<el-select v-model="oneFormBatch[0][item.title]"
												:placeholder="`请选择${item.title}`" clearable>
												<el-option v-for="val in attrs[scope.column.index].detail"
													:key="val.value" :label="val.value" :value="val.value">
												</el-option>
											</el-select>
										</div>
									</template>
									...
									<template v-else-if="item.slot === 'price'">
										<el-input-number :controls="false" v-model="oneFormBatch[0].price" :min="0"
											:max="9999999999" class="priceBox" clearable></el-input-number>
									</template>
									<template v-else-if="item.slot === 'cost'">
										<el-input-number :controls="false" v-model="oneFormBatch[0].cost" :min="0"
											:max="9999999999" class="priceBox" clearable></el-input-number>
									</template>
									<template v-else-if="item.slot === 'ot_price'">
										<el-input-number :controls="false" v-model="oneFormBatch[0].ot_price" :min="0"
									...
									<template v-else-if="item.slot === 'stock'">
										<el-input-number :controls="false" v-model="oneFormBatch[0].stock"
											:disabled="formValidate.virtual_type == 1" :min="0" :max="9999999999"
											class="priceBox" clearable></el-input-number>
									</template>
									...
									<template v-else-if="item.slot === 'weight'">
										<el-input-number :controls="false" v-model="oneFormBatch[0].weight" :step="0.1"
											:min="0" :max="9999999999" class="priceBox" clearable></el-input-number>
									</template>
									<template v-else-if="item.slot === 'volume'">
										<el-input-number :controls="false" v-model="oneFormBatch[0].volume" :step="0.1"
											:min="0" :max="9999999999" class="priceBox" clearable></el-input-number>
									</template>
								...
								<template v-else>
									...
									<template v-if="item.slot === 'price'">
										<el-input-number :controls="false"
											v-model="manyFormValidate[scope.$index].price" :min="0" :max="9999999999"
											class="priceBox"></el-input-number>
									</template>
									<template v-else-if="item.slot === 'cost'">
										<el-input-number :controls="false" v-model="manyFormValidate[scope.$index].cost"
											:min="0" :max="9999999999" class="priceBox"></el-input-number>
									</template>
									<template v-else-if="item.slot === 'ot_price'">
										<el-input-number :controls="false"
											v-model="manyFormValidate[scope.$index].ot_price" :min="0" :max="9999999999"
											class="priceBox"></el-input-number>
									</template>
									<template v-else-if="item.slot === 'stock'">
										<el-input-number :controls="false"
											...
											:disabled="formValidate.virtual_type == 1" :min="0" :max="9999999999"
											:precision="0" class="priceBox"></el-input-number>
									</template>
									<template v-else-if="item.slot === 'bar_code'">
									...
									<template v-else-if="item.slot === 'weight'">
										<el-input-number :controls="false"
											v-model="manyFormValidate[scope.$index].weight" :min="0" :max="9999999999"
											class="priceBox"></el-input-number>
									</template>
									<template v-else-if="item.slot === 'volume'">
										<el-input-number :controls="false"
											v-model="manyFormValidate[scope.$index].volume" :min="0" :max="9999999999"
											class="priceBox"></el-input-number>
									</template>
									<template v-else-if="item.slot === 'fictitious'">
										<el-button
											...
											v-db-click
											@click="addGoodsCoupon(scope.$index, 'manyFormValidate')">选择优惠券</el-button>
										<span class="see"
											v-else-if="manyFormValidate[scope.$index].coupon_id && formValidate.virtual_type == 2"
											v-db-click
											@click="see(manyFormValidate[scope.$index], 'manyFormValidate', scope.$index)">{{ manyFormValidate[scope.$index].coupon_name }}</span>
										<el-button v-else-if="
                      ...
                      " v-db-click @click="addVirtual(scope.$index, 'manyFormValidate')">添加卡密</el-button>
                        ...
                        formValidate.virtual_type == 1
                      " v-db-click @click="see(manyFormValidate[scope.$index], 'manyFormValidate', scope.$index)">已设置</span>
									</template>

									<template v-else-if="item.slot === 'selected_spec'">
										<el-switch v-model="manyFormValidate[scope.$index].is_default_select"
			...
			<el-col :span="24">
				<el-form-item label="库存：">
					<el-input-number :controls="false" v-model="oneFormValidate[0].stock" :min="0" :max="9999999999"
						:disabled="formValidate.virtual_type == 1" :precision="0"
