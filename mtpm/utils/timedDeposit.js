...
/** 展示用：保证金保留两位小数 */
export function formatDepositMoney(amount, fallback = 0) {
...
export function applyTimedDepositToVm(vm, amount, options = {}) {
	...
	vm.deposit_amount_display = formatDepositMoney(amt);
	if (vm.margin !== undefined) {
		vm.margin = formatDepositMoney(amt);
	}
...
export function mergePlateDepositDetails(vm, data) {
	...
	if (data.plate_registered !== undefined) {
		vm.plate_registered = !!data.plate_registered;
	}
	if (data.has_plate !== undefined) {
		vm.has_plate = !!data.has_plate;
	}
	if (data.deposit_sufficient !== undefined) {
		vm.deposit_sufficient = !!data.deposit_sufficient;
	}
	if (data.need_assign_plate !== undefined) {
		vm.need_assign_plate = !!data.need_assign_plate;
	}
}

/** 保证金已足但号牌尚未分配（应点「获取号牌」而非支付） */
export function needAssignPlate(vm) {
	return !!(vm.plate_registered && vm.deposit_sufficient && !vm.has_plate && !vm.plate);
}
