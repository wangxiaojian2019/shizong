			...
			<view class="index">
				...
				<block v-for="(item, index) in styleConfig" :key="index">
					...
					<promotionList ref="promotionLists" v-if="item.name == 'promotionList'" :dataConfig="item"
						:productVideoStatus="product_video_status" :positionTop="positionTop"></promotionList>
					<seckill v-if="item.name == 'seckill'" :dataConfig="item"></seckill>
					<!-- 轮播图-->
					<swiperBg v-if="item.name == 'swiperBg'" :dataConfig="item" :SpecialEvent="SpecialEvent" :timedMeta="timedMeta" :synchronous="synchronous" @onButtonClick="getSpecialEvent" @onButtonClick1="getsynchronization"></swiperBg>
				...
				<!-- #endif -->
					...
					<!-- #ifndef APP-PLUS -->
					<view v-if="configData && configData.record_No" class="site-config"
						@click="goICP(configData.icp_url)">{{ configData.record_No }}</view>
					<view class="site-config" v-if="configData && configData.network_security"
					...
					<!--<view style="width: 70%;margin: 0 auto;text-align: center;padding: 30px 0;">所有的信息由北京明廷国际拍卖有限公司提供技术支持</view>-->
			...
			downTime() {
				...
				this.intervalId = setInterval(() => {
					for (let i = 0; i < that.SpecialEvent.length; i++) {
						...
						that.downTime1[i] = `${d}天${h}时${m}分${s}秒`;
			...
			getProductList(data) {
				...
				if (this.sortAll.length > 0) {
					...
					getCategoryList().then((res) => {
						...
						this.sortAll = parsed.list;
						parsed.list.forEach((el, index) => {
							if (el.id == data) {
								this.sortList = el;
								this.sid = el.children && el.children.length ? el.children[0].id : '';
							}
						});
						this.goodList = [];
						this.goodPage = 1;
			...
			getSpecialEvent() {
				...
				getCategoryList().then((res) => {
					...
					for (let i = 0; i < that.SpecialEvent.length; i++) {
						...
						this.downTime1[i] = `${d}天${h}时${m}分${s}秒`;
						...
						this.SpecialEvent[i].startTime1 = `${month}月${day}日 ${hours}:${minutes}`;
