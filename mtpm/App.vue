	...
	export default {
		...
		methods: {
			initWebSocket() {
				...
				const wsUrl = 'wss://mtpm.billionwz.com/ws';
				if (!wsUrl) {
					return;
				}
				...
				this.globalData.socketTask.onOpen((res) => {
					this.globalData.wsReconnectDelay = 5000;
					console.log('WebSocket 连接成功');
					if (this.globalData.type) {
						this.globalData.socketTask.send({
							data: JSON.stringify(this.globalData.type),
							success: () => {
								console.log("发送成功1");
							},
							fail: (err) => {
				...
				this.globalData.socketTask.onMessage((res) => {
					...
					const myArray = JSON.parse(res.data);
					//console.log(myArray);
				...
				this.globalData.socketTask.onError(() => {
					this.scheduleWsReconnect();
				});

				this.globalData.timer = setInterval(() => {
					if (this.globalData.socketTask && this.globalData.socketTask.readyState === this.globalData
						.socketTask.OPEN) {
						this.globalData.socketTask.send({
							data: JSON.stringify({
								type: 'ping',
			...
			scheduleWsReconnect() {
				...
				const delay = this.globalData.wsReconnectDelay || 5000;
				this.globalData.wsReconnectTimer = setTimeout(() => {
					this.globalData.wsReconnectTimer = null;
					this.globalData.wsReconnectDelay = Math.min(delay * 2, 60000);
					this.initWebSocket();
				}, delay);
			},
