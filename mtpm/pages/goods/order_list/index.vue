<template>
	<view :style="colorStyle">
		<view class="my-order" :class="{ 'has-batch-footer': batchPayMode && orderList.length }">
			...
			<view class="list">
				<view class="item acea-row row-top" v-for="(item, index) in orderList" :key="index">
					<view class="batch-check-wrap acea-row row-center-wrapper" v-if="batchPayMode && canBatchSelect(item)" @click.stop="toggleSelectOrder(item.order_id)">
						<text class="iconfont sel-icon" :class="isOrderSelected(item.order_id) ? 'icon-xuanzhong11 cart-color' : 'icon-weixuan'"></text>
					</view>
					<view class="ord-body">
					<view class="ord-main" @click="goOrderDetails(item.order_id)">
					...
					<view class="bottom acea-row row-right row-middle" @click.stop>
			...
			<view class="batch-pay-footer acea-row row-between-wrapper row-middle" v-if="batchPayMode && orderList.length">
				<view class="acea-row row-middle" @click="toggleSelectAllPayable">
					<text class="iconfont sel-icon" :class="allPayableSelected ? 'icon-xuanzhong11 cart-color' : 'icon-weixuan'"></text>
					<text class="footer-all">{{ $t(`全选`) }}</text>
				</view>
				<view class="acea-row row-middle">
					<view class="footer-sum">
						{{ $t(`合计`) }}：<text class="money">{{ $t(`￥`) }}{{ batchSelectedTotal }}</text>
						<text class="footer-tip">（{{ selectedPayOrderIds.length }}{{ $t(`单`) }}）</text>
					</view>
					<view class="footer-pay bg-color" :class="{ disabled: selectedPayOrderIds.length < 2 }" @click="mergePay">{{ $t(`合并支付`) }}</view>
				</view>
			</view>
...
<script>
import { getOrderList, orderData, orderCancel, orderDel, orderBatchPayPrepare } from '@/api/order.js';
...
export default {
	...
	computed: {
		...
		payableOrderIdsOnPage() {
			return this.orderList
				.filter((item) => this.canBatchSelect(item))
				.map((item) => item.order_id);
		},
		allPayableSelected() {
			const ids = this.payableOrderIdsOnPage;
			return ids.length > 0 && ids.every((id) => this.selectedPayOrderIds.includes(id));
		},
		batchSelectedTotal() {
			let t = 0;
			this.orderList.forEach((item) => {
				if (this.selectedPayOrderIds.includes(item.order_id)) {
	...
	methods: {
						...
						orderCancel(order_id)
							.then((res) => {
								return that.$util.Tips(
									...
									function () {
										that.selectedPayOrderIds = that.selectedPayOrderIds.filter((id) => id !== order_id);
		...
		toggleSelectOrder(orderId) {
			const i = this.selectedPayOrderIds.indexOf(orderId);
			if (i === -1) this.selectedPayOrderIds.push(orderId);
			else this.selectedPayOrderIds.splice(i, 1);
		},
		toggleSelectAllPayable() {
			const ids = this.payableOrderIdsOnPage;
			if (!ids.length) return;
			if (this.allPayableSelected) this.selectedPayOrderIds = [];
			else this.selectedPayOrderIds = [...ids];
		},
		mergePay() {
			const that = this;
			...
			orderBatchPayPrepare({
				order_ids: that.selectedPayOrderIds.join(',')
			})
				.then((res) => {
					const batchNo = res.data.batch_no;
					if (!batchNo) {
						return that.$util.Tips({ title: that.$t(`创建合并支付失败`) });
					}
					uni.navigateTo({
						url: `/pages/goods/cashier/index?order_id=${batchNo}&from_type=order_batch`
					});
				})
				.catch((err) => {
					return that.$util.Tips({
...
.my-order .list .item .ord-body {
	flex: 1;
	min-width: 0;
}

.my-order .list .item .batch-check-wrap {
	width: 56rpx;
	flex-shrink: 0;
	align-self: stretch;
	padding-top: 36rpx;
...
.my-order .list .item .batch-check-wrap .sel-icon {
	font-size: 40rpx;
	color: #ccc;
}
...
.batch-pay-footer .footer-all {
	margin-left: 10rpx;
	color: #282828;
}

.batch-pay-footer .footer-sum {
	margin-right: 8rpx;
	text-align: right;
}

.batch-pay-footer .footer-sum .money {
	font-size: 30rpx;
	font-weight: bold;
	color: var(--view-priceColor);
}

.batch-pay-footer .footer-tip {
	font-size: 22rpx;
	color: #999;
	margin-left: 6rpx;
}

.batch-pay-footer .footer-pay {
	padding: 0 32rpx;
	height: 72rpx;
	line-height: 72rpx;
	border-radius: 36rpx;
...
.batch-pay-footer .footer-pay.disabled {
	opacity: 0.45;
	pointer-events: none;
}
</style>