<template>
	<view v-if="!load">
		...
		<view class="w-full bottomBox">
			<view class="w-full plate" v-if="plate">号牌{{plate}},<text v-if="state==2">退款审核中</text><text
					v-else>保证金已缴纳</text></view>
			<view class="w-full plate" v-else-if="plate_registered">已办理号牌，<text v-if="needAssignPlate">保证金已缴足，请点击下方获取号牌</text><text
					v-else-if="state==0">请缴纳保证金（缴足后自动分配号牌）</text><text
					v-else-if="state==2">退款审核中</text><text v-else>号牌分配中</text></view>
			<view class="plateBox">

				<view class="w-full plateBtn" v-if="!plate_registered && cate.status!=2" @click="addNumberPlate">办理号牌</view>
				<view class="w-full plateBtn" style="background-color: #e5e5e5;" v-else-if="plate_registered && state==2">退款审核中</view>
				<view class="w-full plateBtn" v-else-if="needAssignPlate && cate.status!=2" @click="retryAssignPlate">获取号牌</view>
				<view class="w-full plateBtn" v-else-if="plate_registered && state==0 && cate.status!=2" @click="pay()">缴纳保证金</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			/** 跳转 Lot 后同步倒计时：专场 current_lot 与当前 index 一致即视为进行中 */
			syncAuctionTimer() {
				if (!this.cate || !this.detail) {
					return;
				}
				const ended = Number(this.detail.presale_end_time || 0) > 0;
				if (ended) {
					if (this.timer) {
						clearInterval(this.timer);
						this.timer = null;
				...
				const lotLive = Number(this.cate.status) === 1 && Number(this.cate.number) === Number(this.index);
				const productStarted = Number(this.detail.presale_start_time || 0) > 0;
				if (lotLive || productStarted) {
					this.downTrue = 1;
					this.downTime();
				}
			},

			/** 专场内是否还有下一件拍品（当前 Lot 不是最后一件） */
			hasNextLot() {
			...
			addNumberPlate() {
				...
				getNumberPlate(that.cate_id).then((res) => {
					...
					that.getPlateNumberdetails(msg);
				})
			},
			...
			retryAssignPlate() {
				const that = this;
				uni.showLoading({ title: '分配号牌中' });
				that.getPlateNumberdetails('retry').finally(() => {
					uni.hideLoading();
					if (that.plate) {
						uni.showToast({ title: '您的号牌是' + that.plate, icon: 'none', duration: 2000 });
					} else if (that.needAssignPlate) {
						uni.showToast({ title: '号牌分配失败，请稍后重试或联系客服', icon: 'none', duration: 2500 });
					}
				});
			},

			getPlateNumberdetails(applyMsg) {
				const that = this;
				return getNumberdetails(that.cate_id).then((res) => {
					...
					if (applyMsg && applyMsg !== 'retry') {
						let tip = applyMsg;
						if (that.plate) {
							tip = '您的号牌是' + that.plate;
						} else if (that.plate_registered && that.state == 0) {
							tip = tip || '请缴纳保证金，缴足后自动分配号牌';
						}
						uni.showToast({
							title: tip,
							icon: 'none',
			...
			pay() {
				const that = this;
				if (needAssignPlate(that)) {
					that.retryAssignPlate();
					return;
				}
			...
			wxpay(paymentParams) {
				...
				uni.requestPayment({
					...
					success: (res) => {
						...
						setTimeout(() => that.getPlateNumberdetails(), 2000);
						uni.showToast({
							title: '支付成功'
						});
