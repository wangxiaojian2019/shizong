	...
	<view class="w-full" style="height: 100vh;">
		<view v-if="!load">
			...
			<view class="w-full bottomBox">
				...
				<view class="plateBox">
					...
					<view class="w-full plateBtn" v-else-if="needAssignPlate" @click="retryAssignPlate">获取号牌</view>
					<view class="w-full plateBtn" v-else-if="plate_registered && state==0" @click="payShow()">缴纳保证金</view>
		...
		<u-popup v-model="popupShow" mode="center" width="90%" border-radius="10">
			...
			<scroll-view scroll-y="true" style="height: 400px;width: 90%;margin: 0 auto;" class="bold">
				...
				<view class="text-indent mt-8 ">网络拍卖会专场保证金为{{deposit_amount_display || margin}}元人民币（网络大拍为1万元人民币）。</view>
		...
		methods: {
			...
			addNumberPlate() {
				...
				getNumberPlate(that.id).then((res) => {
					const msg = (res.data && res.data.msg) ? res.data.msg : '办理成功';
					that.getPlateNumberdetails(1, msg);
				})
			},

			getPlateNumberdetails(index, applyMsg) {
				const that = this;
				return getNumberdetails(that.id).then((res) => {
			...
			retryAssignPlate() {
				...
				that.getPlateNumberdetails(1, '').finally(() => {
					uni.hideLoading();
					if (that.plate) {
			...
			pay(index) {
				const that = this;
				if (index == 1 && needAssignPlate(that)) {
			...
			payShow() {
				if (needAssignPlate(this)) {
					this.retryAssignPlate();
					return;
				}
				this.popupShow = true;
			},
