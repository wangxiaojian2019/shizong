...
class CrontabRunServices
{
    ...
    public $markList = [
        ...
        'signRemind' => '未签到提醒',
        'customTimer' => '自定义定时任务',
        'limitedTimeSale' => '开始拍卖',
        'automatic' => '更新拍卖',
        'synchronousAutomatic' => '同步拍拍卖',
    ...
    public function limitedTimeSale(){
        try {
            app()->make(StoreCategoryServices::class)->limitedTimeSale();
            $this->crontabLog(' 自定义定时器执行成功1');
        } catch (\Throwable $e) {
            $this->crontabLog('自定义定时器1执行失败,失败原因:' . $e->getMessage());
        }
    }
    
     public function auction(){
        try {
            app()->make(StoreCategoryServices::class)->auction();
            $this->crontabLog(' 自定义定时器执行成功2');
        } catch (\Throwable $e) {
            $this->crontabLog('自定义定时器2执行失败,失败原因:' . $e->getMessage());
        }
    }
    
    public function automatic(){
        try {
            app()->make(StoreBiddingServices::class)->automatic();
            $this->crontabLog(' 自定义定时器执行成功3');
        } catch (\Throwable $e) {
            $this->crontabLog('自定义定时器3执行失败,失败原因:' . $e->getMessage());
        }
    }
    
    public function synchronousAutomatic(){
        try {
            app()->make(StoreBiddingServices::class)->synchronousAutomatic();
            $this->crontabLog('同步拍代理出价成功');
        } catch (\Throwable $e) {
            $this->crontabLog('同步拍代理出价失败,失败原因:' . $e->getMessage());
        }
    }
    
     public function sendSubscribeMessage(){
        try {
            app()->make(StoreCategoryServices::class)->sendSubscribeMessage();
            $this->crontabLog('同步拍代理出价成功');
        } catch (\Throwable $e) {
            $this->crontabLog('同步拍代理出价失败,失败原因:' . $e->getMessage());
        }
