...
class StoreOrderServices extends BaseServices
{
    ...
    public function getOrderData(int $uid = 0)
    {
        ...
        $data['unpaid_count'] = (string)$this->dao->count(['status' => 0, 'pid' => 0] + $countWhere);
    ...
    public function orderUnpaidCancel()
    {
        ...
        foreach ($list as $order) {
            ...
            if (($order['add_time'] + bcmul($secs, '3600', 0)) < time()) {
                try {
                    $cancelOk = $this->transaction(function () use ($order, $refundServices) {
                        //回退积分和优惠卷
                    ...
                    if ($cancelOk) {
                        /** @var \app\services\product\product\StoreTimedDepositServices $timedDepositServices */
                        $timedDepositServices = app()->make(\app\services\product\product\StoreTimedDepositServices::class);
                        $deductResult = $timedDepositServices->tryDeductForUnpaidOrder((int)$order['id'], $order);
                        if (!$deductResult['ok'] && empty($deductResult['skipped'])) {
                            Log::error('限时拍未支付订单违约扣减失败,订单号:' . $order['order_id'] . ',原因:' . ($deductResult['msg'] ?? ''));
                        }
                    }
...
//            $order['code'] = $url;
     ...
     * @return int
     */
    protected function getOrderPayInvalidTime(array $orderRow): int
    {
        $orderCancelTime = (int)sys_config('order_cancel_time', 0);
        $orderActivityTime = (int)sys_config('order_activity_time', 0);
        $addTime = (int)($orderRow['add_time'] ?? 0);
        if (!empty($orderRow['combination_id'])) {
            $hours = (int)(sys_config('order_pink_time', 0) ?: $orderActivityTime);
        } elseif (!empty($orderRow['seckill_id'])) {
            $hours = (int)(sys_config('order_seckill_time', 0) ?: $orderActivityTime);
        } elseif (!empty($orderRow['bargain_id'])) {
            $hours = (int)(sys_config('order_bargain_time', 0) ?: $orderActivityTime);
        } else {
            $hours = $orderCancelTime;
        }
        $time = $hours * 3600 + $addTime;
        return $time < 0 ? 0 : $time;
    }

    /**
     * 同一用户、同一批订单再次「合并预创建」时，使旧 batch 缓存与守卫失效，避免多设备/多标签页并存两个可支付 batch。
     * 守卫键与订单号绑定；其它用户无法通过守卫命中他人缓存（ORDER_BATCH_PAY 内仍有 uid 校验）。
     */
    public static function invalidateUserBatchCacheForOrderIds(int $uid, array $orderIds): void
    {
        $clearedBatchNos = [];
        foreach ($orderIds as $oid) {
            $oid = (string)$oid;
            $gk = 'ORDER_BATCH_GUARD_' . $oid;
            $oldBatch = CacheService::get($gk);
            if (!$oldBatch || !is_string($oldBatch)) {
                continue;
            }
            // 同一轮 prepare 里多个 order_id 可能同属一个旧 batch，避免重复删缓存但须清掉当前 oid 的守卫
            if (isset($clearedBatchNos[$oldBatch])) {
                CacheService::delete($gk);
                continue;
            }
            $payload = CacheService::get('ORDER_BATCH_PAY_' . $oldBatch);
            if ($payload && (int)($payload['uid'] ?? 0) === $uid) {
                CacheService::delete('ORDER_BATCH_PAY_' . $oldBatch);
                // 旧 batch 里所有订单的守卫一并删除，避免只清了参与新合并的 oid、其余仍指向已删 batch（业务不一致）
                if (!empty($payload['order_ids']) && is_array($payload['order_ids'])) {
                    foreach ($payload['order_ids'] as $fid) {
                        CacheService::delete('ORDER_BATCH_GUARD_' . (string)$fid);
                    }
                }
                $clearedBatchNos[$oldBatch] = true;
            } else {
                // 守卫指向空缓存或非本人 batch（异常数据）：只删本 order 守卫
                CacheService::delete($gk);
            }
        }
    }

    /** 合并支付完成或删除缓存后释放守卫，避免 order_id 长期指向已过期的 batch_no */
    public static function releaseOrderBatchGuards(array $orderIds): void
    {
        foreach ($orderIds as $oid) {
            CacheService::delete('ORDER_BATCH_GUARD_' . (string)$oid);
        }
    }

    /**
     * 合并支付：校验订单并写入缓存，返回 batch_no
     * @param int $uid
     * @param array|string $orderIdsInput 订单 order_id 列表或逗号分隔字符串
     * @return array
     */
    public function prepareOrderBatchPay(int $uid, $orderIdsInput): array
    {
        if (is_string($orderIdsInput)) {
            $orderIds = array_values(array_filter(array_map('trim', explode(',', $orderIdsInput))));
        } elseif (is_array($orderIdsInput)) {
            $orderIds = array_values(array_filter($orderIdsInput));
        } else {
            throw new ApiException('参数错误');
        }
        if (count($orderIds) < 2) {
            throw new ApiException('请至少选择2个订单合并支付');
        }
        if (count($orderIds) > 30) {
            throw new ApiException('单次最多合并30个订单');
        }
        ...
        $total = '0';
        $totalPostage = '0';
        $invalidTime = PHP_INT_MAX;
        foreach ($orderIds as $oid) {
            $order = $this->dao->get(['order_id' => $oid, 'uid' => $uid, 'is_del' => 0, 'is_system_del' => 0]);
            if (!$order) {
                throw new ApiException('订单不存在');
            }
            $row = is_array($order) ? $order : $order->toArray();
            if (!empty($row['paid']) || !empty($row['is_cancel'])) {
                throw new ApiException('存在已支付或已取消的订单，请刷新后重试');
            }
            if ((int)($row['refund_status'] ?? 0) !== 0) {
                throw new ApiException('存在退款中的订单，无法合并支付');
            }
            if ((int)($row['pid'] ?? 0) !== 0) {
                throw new ApiException('子订单不支持合并支付');
            }
            if ($row['pink_id'] && $pinkServices->isPinkStatus($row['pink_id'])) {
                throw new ApiException('拼团订单暂不支持合并支付');
            }
            $total = bcadd($total, (string)$row['pay_price'], 2);
            $totalPostage = bcadd($totalPostage, (string)($row['pay_postage'] ?? 0), 2);
            $t = $this->getOrderPayInvalidTime($row);
            if ($t < $invalidTime) {
                $invalidTime = $t;
            }
        }
        if ($invalidTime === PHP_INT_MAX) {
            $invalidTime = time() + 3600;
        }
        self::invalidateUserBatchCacheForOrderIds($uid, $orderIds);
        $batchNo = 'bp' . strtoupper(substr(md5(uniqid((string)$uid, true)), 0, 18));
        $payload = [
            'uid' => $uid,
            'order_ids' => $orderIds,
            'total_price' => $total,
        ...
        CacheService::set('ORDER_BATCH_PAY_' . $batchNo, $payload, 1800);
        foreach ($orderIds as $oid) {
            CacheService::set('ORDER_BATCH_GUARD_' . (string)$oid, $batchNo, 1800);
        }
        return [
            'batch_no' => $batchNo,
            'pay_price' => $total,
     ...
     * 收银台展示信息
     * @param int $uid
     * @param string $orderId 单笔订单号或合并支付的 batch_no
     * @param string $type order | order_batch | svip | recharge
     * @return array
     */
    public function getCashierInfo(int $uid, string $orderId, string $type)
    {
        ...
        switch ($type) {
            case 'order_batch':
                $cache = CacheService::get('ORDER_BATCH_PAY_' . $orderId);
                if (!$cache || (int)($cache['uid'] ?? 0) !== $uid) {
                    throw new PayException('合并支付已失效，请返回订单列表重新选择');
                }
                $ids0 = $cache['order_ids'] ?? [];
                foreach ($ids0 as $oid) {
                    if (CacheService::get('ORDER_BATCH_GUARD_' . (string)$oid) !== $orderId) {
                        throw new PayException('合并支付已失效，请返回订单列表重新选择');
                    }
                }
                $data['friend_pay_status'] = 0;
                $data['pay_price'] = $cache['total_price'];
                $data['pay_postage'] = $cache['total_postage'] ?? '0';
                $data['offline_postage'] = (int)sys_config('offline_postage', 0);
                $data['invalid_time'] = (int)($cache['invalid_time'] ?? time());
                $data['oid'] = 0;
                $data['is_gift'] = 0;
                $data['is_batch_pay'] = 1;
                break;
