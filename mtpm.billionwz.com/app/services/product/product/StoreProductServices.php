...
class StoreProductServices extends BaseServices
{
    ...
    public function getList(array $where)
    {
        ...
        foreach ($list as &$item) {
            ...
            $item['plate'] = $storeNumberPlateDao->selectPlate(['user_id' =>$item['uid'],'cate_id' => $item['cate_id']]);
        }
...
//                '(SELECT SUM(sales) FROM `' . $prefix . 'store_product_attr_value` WHERE `product_id` = `' . $prefix . 'store_product`.`id` AND `type` = 0) as sales',
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function productDetail(Request $request, int $id, int $type,int $uid)
    {
        $uid = (int)$request->uid();
        $data['uid'] = $uid;
        ...
        $storeInfo['presale_start_time'] = date('Y-m-d H:i:s', $storeInfo['presale_start_time']);
        $storeInfo['presale_end_time'] = date('Y-m-d H:i:s', $storeInfo['presale_end_time']);
        ...
        $data['bidding'] = $StoreBiddingDao->getList(['product_id'=> $id]);
       // $status = 1;
        $wherex = [
            'product_id' => $id, 
            'uid' => $uid,
        ...
        $data['agent'] = $StoreAgentPriceDao->selectAgent($wherex);
        if(!$data['agent']){
            $wherex = [
                'product_id' => $id, 
                'uid' => $uid,
...
//            }
    ...
    public function getproductLists($where,$page,$limit){
       return $this->dao->getproductList($where,$page,$limit);
    }
}
