...
class OrderPayServices
{
    ...
    public function beforePay(array $orderInfo, string $payType, array $options = [])
    {
        ...
        if (isset($orderInfo['member_type'])) {
            ...
            $title = $orderInfoServices->getCarIdByProductTitle((int)$orderInfo['id']);
            // 合并支付沿用首单 oid 取摘要；若购物车摘要为空则兜底，避免与单笔支付一致地下单失败（410276）
            if ($title === '' && !empty($orderInfo['is_batch_pay'])) {
                $title = '合并订单';
            }
            $body = Str::substrUTf8($site_name . '--' . $title, 20);
            $successAction = empty($orderInfo['is_batch_pay']) ? 'product' : 'product_batch';
            /** @var StoreOrderServices $orderServices */
            $orderServices = app()->make(StoreOrderServices::class);
            if (empty($orderInfo['is_batch_pay'])) {
