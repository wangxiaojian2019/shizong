...
class YuePayServices extends BaseServices
{
    ...
    /**
     * 多订单合并余额支付：扣款一次，逐单入账并标记支付成功
     * @param array $orders StoreOrder 行数组列表
     */
    public function yueBatchOrderPay(array $orders, int $uid): array
    {
        if (!$orders) {
            throw new ApiException(410173);
        }
        ...
        $total = '0';
        foreach ($orders as $orderInfo) {
            $total = bcadd($total, (string)$orderInfo['pay_price'], 2);
        }
        /** @var UserServices $services */
        $services = app()->make(UserServices::class);
        $userInfo = $services->getUserInfo($uid);
        if ($userInfo['now_money'] < $total) {
            return ['status' => 'pay_deficiency', 'msg' => '余额不足' . floatval($total)];
        }
        $this->transaction(function () use ($services, $orders, $userInfo, $total, $uid) {
            $res = false !== $services->bcDec($userInfo['uid'], 'now_money', $total, 'uid');
            /** @var UserMoneyServices $userMoneyServices */
            $userMoneyServices = app()->make(UserMoneyServices::class);
            $runningMoney = (string)$userInfo['now_money'];
            /** @var StoreOrderSuccessServices $orderServices */
            $orderServices = app()->make(StoreOrderSuccessServices::class);
            foreach ($orders as $orderInfo) {
                $runningMoney = bcsub($runningMoney, (string)$orderInfo['pay_price'], 2);
                $number = $orderInfo['pay_price'];
                $res = $res && $userMoneyServices->income('pay_product', $userInfo['uid'], $number, $runningMoney, $orderInfo['id']);
                $res = $res && $orderServices->paySuccess($orderInfo, PayServices::YUE_PAY);
            }
            if (!$res) {
                throw new ApiException(410279);
            }
