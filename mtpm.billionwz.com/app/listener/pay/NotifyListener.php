        ...
        if (isset($notify['out_bill_no']) && $notify['out_bill_no']) {
            ...
            if (isset($notify['attach']) && $notify['attach']) {
                ...
                if ($notify['attach'] === 'product_batch') {
                    return app()->make(PayNotifyServices::class)->wechatProductBatch($notify['out_trade_no'], $notify['transaction_id'] ?? '', $payType);
                }
                return (new Hook(PayNotifyServices::class, 'wechat'))->listen($notify['attach'], $notify['out_trade_no'], $notify['transaction_id'], $payType);
            }

            if (isset($notify['attach']) && $notify['attach'] === 'wechat' && isset($notify['out_trade_no'])) {
