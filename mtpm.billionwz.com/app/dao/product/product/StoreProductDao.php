...
class StoreProductDao extends BaseDao
{
    ...
    public function getList(array $where, int $page = 0, int $limit = 0, string $order = '')
    {
        ...
        return $this->search($where, false)->order(($order ? $order . ' ,' : '') . 'sort asc,id desc')
    ...
    public function getInfoone(int $id)
    {
        return $this->search()->with('coupons')->find($id)->toArray();
    }
    ...
    public function automatic($where,$data){
    ...
    /**
     * 批量统计用户在各专场下的商品数（与 getproduct 条件一致，避免后台列表 N+1）
     * @param array $pairs [['uid'=>1,'cate_id'=>3], ...]
     * @param bool $salesFilter 是否追加 sales=1
     * @return array key 为 "{uid}_{cate_id}" => int
     */
    public function batchCountByUidCate(array $pairs, bool $salesFilter = false): array
    {
        $keys = [];
        $uids = [];
        $cateIds = [];
        foreach ($pairs as $pair) {
            ...
            $keys[$mapKey] = ['uid' => $uid, 'cate_id' => $cateId];
            $uids[$uid] = $uid;
            $cateIds[$cateId] = $cateId;
        }
        $result = array_fill_keys(array_keys($keys), 0);
        if (!$keys) {
            return $result;
        }
        ...
        $caseParts = [];
        $i = 0;
        foreach ($keys as $mapKey => $pair) {
            $uid = (int)$pair['uid'];
            $cid = (int)$pair['cate_id'];
            $caseParts[] = 'SUM(CASE WHEN `uid`=' . $uid . ' AND FIND_IN_SET(' . $cid . ',`cate_id`) THEN 1 ELSE 0 END) AS `c_' . $i . '`';
            $i++;
        }

        $query = $this->getModel()->whereIn('uid', array_values($uids));
        if ($salesFilter) {
            $query->where('sales', 1);
        }
        $cateIdList = array_values($cateIds);
        $query->where(function ($q) use ($cateIdList) {
            $parts = [];
            foreach ($cateIdList as $cid) {
                $parts[] = 'FIND_IN_SET(' . (int)$cid . ',`cate_id`)';
            }
            $q->whereRaw('(' . implode(' OR ', $parts) . ')');
        });
        ...
        $rows = $query->fieldRaw(implode(',', $caseParts))->select()->toArray();
        if (!$rows) {
            return $result;
        }
        $rowData = $rows[0];
        $idx = 0;
        foreach (array_keys($keys) as $mapKey) {
            $alias = 'c_' . $idx;
            $result[$mapKey] = (int)($rowData[$alias] ?? 0);
            $idx++;
        }
        return $result;
    }
    
    public function getproductList($where,$page,$limit){
        return $this->search($where)->where('presale_end_time','<>',0)->page($page, $limit)->order('id desc')->select()->toArray();
    }
}
