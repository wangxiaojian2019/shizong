...
class StoreCategoryDao extends BaseDao
{
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getALlByIndex( $limit = 1)
    {
        return $this->getModel()->where('is_show', 1)->where('model', 0)->order('sort DESC')->limit($limit)->select()->toArray();
    }

    /**
     * 后台拍卖周期绑定：全部顶级限时拍专场（不限是否展示，绑定保存时再校验展示）
     * @return array
     */
    public function listTimedAuctionForSessionBind(): array
    {
        return $this->getModel()
            ->where('pid', 0)
            ->whereRaw('(model = 0 OR model = \'0\' OR model IS NULL)')
            ->field('id,cate_name,status,is_show,sort,model,pid')
            ->order('sort desc,id desc')
            ->select()
            ->toArray();
    }
    
    public function getSynchronization($limit = 1)
    {
        return $this->getModel()->where('is_show', 1)->where('model', 1)->order('sort DESC')->limit($limit)->select()->toArray();
    }

    /**
     * @deprecated 周期绑定已迁至 eb_store_auction_session.cate_a_id / cate_b_id
     */
    public function getTimedBySessionId(int $sessionId): array
    {
        if ($sessionId <= 0) {
            return [];
        }
        return app()->make(\app\services\product\product\StoreAuctionSessionServices::class)
            ->getSessionCategories($sessionId);
    }
    
   public function getALlIndex($status = 0)
    {
        return $this->getModel()->where('status', $status)->where('is_show', 1)->order('sort DESC')->select()->toArray();
    }
    
    public function updateCate($id = 0,array $data)
    {
        $this->getModel()->where('id', $id)->save($data);
    }
