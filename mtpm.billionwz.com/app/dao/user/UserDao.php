...
class UserDao extends BaseDao
{
    ...
    public function allUser(){
       return $this->getModel()->where('phone', '<>', '')->select()->toArray();
    }
    
    
    public function updateMargin($uid,$price,$status){
        
        $model = $this->search(['uid' => $uid])->find()->toArray();
        if($status==0){
            $model['margin'] = $model['margin'] + $price;
        }else{
            $model['margin'] = $model['margin'] - $price;
        }
        
        //$model->save();
        $this->search(['uid' => $uid])->update($model);
        return $model;
    }
}