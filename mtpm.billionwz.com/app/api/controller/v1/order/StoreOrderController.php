...
class StoreOrderController
{
    ...
    /**
     * 合并支付：提交待支付订单 order_id 列表，生成 batch_no
     */
    public function batchPayPrepare(Request $request)
    {
        [$orderIds] = $request->postMore([['order_ids', '']], true);
        return app('json')->success($this->services->prepareOrderBatchPay((int)$request->uid(), $orderIds));
    }
    ...
    public function pay(Request $request, StorePinkServices $services, OrderPayServices $payServices, YuePayServices $yuePayServices)
    {
        ...
        $isBatchUni = strlen($uni) >= 2 && strtolower(substr($uni, 0, 2)) === 'bp';
        $batchCacheEarly = CacheService::get('ORDER_BATCH_PAY_' . $uni);
        $payLock = CacheService::get('PAY_LOCK_' . $uni);
        if ($payLock) {
            return app('json')->fail('订单支付中，请勿重复支付');
        }
        // 仅当合并缓存仍存在时延长互斥（防连点多笔预下单）；失效 bp 仍用 2 秒以免长时间占锁
        CacheService::set('PAY_LOCK_' . $uni, 'PAY_LOCK', $batchCacheEarly ? 45 : 2);

        if ($batchCacheEarly) {
            return $this->batchOrderPay($request, $services, $payServices, $yuePayServices, $uni, $batchCacheEarly, $paytype, $quitUrl, (int)$type);
        }
        if ($isBatchUni) {
            return app('json')->fail('合并支付已失效或已支付，请返回订单列表重新操作');
        }
     ...
     * 合并支付扣款 / 拉起三方支付
     */
    protected function batchOrderPay(
        Request $request,
        StorePinkServices $pinkServices,
        OrderPayServices $payServices,
        YuePayServices $yuePayServices,
        string $batchNo,
        array $batchCache,
        string $paytype,
        string $quitUrl,
        ...
        if ($friendType === 1) {
            return app('json')->fail('合并支付不支持好友代付');
        }
        $uid = (int)$request->uid();
        if ((int)($batchCache['uid'] ?? 0) !== $uid) {
            return app('json')->fail('无权支付');
        }
        if (empty($batchCache['order_ids']) || !is_array($batchCache['order_ids'])) {
            return app('json')->fail('合并支付数据无效，请重新选择订单');
        }
        // 与 prepare 写入的守卫一致；逐单校验防止缓存 order_ids 被篡改后仅首单仍匹配守卫
        foreach ($batchCache['order_ids'] as $oid) {
            if (CacheService::get('ORDER_BATCH_GUARD_' . (string)$oid) !== $batchNo) {
                return app('json')->fail('合并支付会话已失效，请返回订单列表重新合并支付');
            }
        }
        $orderRows = [];
        $total = '0';
        foreach ($batchCache['order_ids'] as $oid) {
            ...
            if (!$order || $order['is_del'] == 1 || $order['is_system_del'] == 1) {
                return app('json')->fail('订单已失效，请返回订单列表重新选择');
            }
            if ((int)$order['uid'] !== $uid) {
                return app('json')->fail('无权支付');
            }
            if ($order['paid'] || $order['is_cancel']) {
                return app('json')->fail('订单状态已变更，请刷新后重试');
            }
            if ((int)($order['refund_status'] ?? 0) !== 0) {
                return app('json')->fail('存在退款相关订单，无法合并支付');
            }
            if ($order['pink_id'] && $pinkServices->isPinkStatus($order['pink_id'])) {
                return app('json')->fail('拼团状态已变更，无法支付');
            }
            $total = bcadd($total, (string)$order['pay_price'], 2);
            $orderRows[] = $order->toArray();
        }
        if (bccomp($total, (string)($batchCache['total_price'] ?? '0'), 2) !== 0) {
            return app('json')->fail('订单金额已变更，请返回重新合并支付');
        }
        ...
        if (bccomp($total, '0', 2) <= 0) {
            foreach ($orderRows as $row) {
                $success->zeroYuanPayment($row, $uid);
            }
            CacheService::delete('ORDER_BATCH_PAY_' . $batchNo);
            StoreOrderServices::releaseOrderBatchGuards($batchCache['order_ids']);
            return app('json')->status('success', '支付成功', ['order_id' => $batchNo, 'key' => '']);
        }

        switch ($paytype) {
            case PayServices::YUE_PAY:
                $pay = $yuePayServices->yueBatchOrderPay($orderRows, $uid);
                if ($pay['status'] === true) {
                    CacheService::delete('ORDER_BATCH_PAY_' . $batchNo);
                    StoreOrderServices::releaseOrderBatchGuards($batchCache['order_ids']);
                    return app('json')->status('success', 410197);
            ...
            case PayServices::OFFLINE_PAY:
                foreach ($batchCache['order_ids'] as $oid) {
                    $this->services->setOrderTypePayOffline($oid);
                    $ord = $this->services->get(['order_id' => $oid]);
                    if ($ord) {
                        event('NoticeListener', [$ord->toArray(), 'admin_pay_success_code']);
                    }
                }
                CacheService::delete('ORDER_BATCH_PAY_' . $batchNo);
                StoreOrderServices::releaseOrderBatchGuards($batchCache['order_ids']);
            ...
            default:
                $first = $orderRows[0];
                $synthetic = $first;
                $synthetic['order_id'] = $batchNo;
                $synthetic['pay_price'] = $total;
                // 与 getCashierInfo、OrderPayServices 一致使用标量真值，走 product_batch / 不写单笔 pay_type
                $synthetic['is_batch_pay'] = 1;
                $synthetic['pay_uid'] = $uid;
                $synthetic['is_channel'] = $this->getChennel[$request->getFromType()] ?? ($request->isApp() ? 0 : 1);
                $payInfo = $payServices->beforePay($synthetic, $paytype, ['quitUrl' => $quitUrl]);
                return app('json')->status($payInfo['status'], $payInfo['payInfo']);
        }
    }
}
