...
Route::group(function () {
    ...
    Route::group(function () {
        ...
        Route::post('order/batch_pay/prepare', 'v1.order.StoreOrderController/batchPayPrepare')->name('orderBatchPayPrepare')->option(['real_name' => '合并支付预创建']); //合并支付
    ...
    Route::group(function () { 
        ...
        Route::get('agentPrice/add', 'v1.store.storeAgentPriceController/add')->name('addagentPrice')->option(['real_name' => '代理出价']); 
        Route::get('agentPrice/updateAgent', 'v1.store.storeAgentPriceController/updateAgent')->name('updateAgent')->option(['real_name' => '更新代理出价']); 
        Route::get('agentPrice/cancelAgent', 'v1.store.storeAgentPriceController/cancelAgent')->name('cancelAgent')->option(['real_name' => '取消代理出价']); 
        
             
        Route::get('agentPrice/storeCount', 'v1.store.storeAgentPriceController/storeCount')->name('storeCount')->option(['real_name' => '统计']); 
        Route::get('user/realname', 'v1.user.UserController/realname')->name('realname')->option(['real_name' => '提交实名认证']);
        Route::get('getBidding', 'v1.store.StoreBiddingController/getBidding')->name('getBidding')->option(['real_name' => '出价记录']);
        Route::get('auction', 'v1.store.StoreBiddingController/auction')->name('auction')->option(['real_name' => '竞价']);
        Route::get('synchronousAddPrice', 'v1.store.StoreBiddingController/synchronousAddPrice')->name('synchronousAddPrice')->option(['real_name' => '同步拍竞价']); 
        Route::get('getPlate', 'v1.store.StoreNumberPlateController/getPlate')->name('getPlate')->option(['real_name' => '获取号牌']);
        Route::get('wxpay', 'v1.store.PayController/index')->name('wxpay')->option(['real_name' => '创建保证金订单']);
        Route::get('getProductList', 'v1.store.StoreProductController/getProductList')->name('wxpay')->option(['real_name' => '我的中拍']);
        Route::get('numberPlate/deposit', 'v1.store.StoreNumberPlateController/depositStatus')->name('depositStatus')->option(['real_name' => '限时拍保证金状态']);
        Route::get('numberPlate/refund', 'v1.store.StoreNumberPlateController/refund')->name('refund')->option(['real_name' => '退款保证金申请']);
    })->option(['mark' => 'plate', 'mark_name' => '号牌户']);
...
Route::group(function () {
    Route::group(function () {
         Route::get('/numberPlate/add/:cate_id', 'v1.store.StoreNumberPlateController/add')->name('add')->option(['real_name' => '办理号牌']);
        //Route::get('bidding/indexs', 'v1.store.StoreBiddingController/indexs')->name('auction')->option(['real_name' => '竞价']); 
    ...
    Route::group(function () {Route::get('/numberPlate/details/:cate_id', 'v1.store.StoreNumberPlateController/details')->name('details')->option(['real_name' => '查询号牌']);
        ...
        Route::get('getsynchronization', 'v1.store.CategoryController/getsynchronization')->name('getsynchronization')->option(['real_name' => '同步拍']);
        ...
        Route::get('productBiddingList/:product_id', 'v1.store.StoreBiddingController/productBiddingList')->name('productBiddingList()')->option(['real_name' => '产品出价记录']);
        
        
         Route::get('wxlogin', 'v1.wechat.Login/index')->name('wxlogin')->option(['real_name' => '获取openid']);
        
        Route::post('wxnotify', 'v1.store.NotifyController/handleNotify')->name('wxnotify')->option(['real_name' => '保证金订单回调']);
        
        Route::get('category', 'v1.store.CategoryController/category')->name('category')->option(['real_name' => '商品分类类']);
        Route::get('category/details/:id', 'v1.store.CategoryController/details')->name('details')->option(['real_name' => '商品分类详情']);
        Route::get('category/getSynchronizationlist/:id', 'v1.store.CategoryController/getSynchronizationlist')->name('getSynchronizationlist')->option(['real_name' => '商品分类详情']);
        Route::get('category/addView/:id', 'v1.store.CategoryController/addView')->name('addView')->option(['real_name' => '围观']);
        Route::get('category/getcatlist', 'v1.store.CategoryController/getcatlist')->name('getcatlist')->option(['real_name' => '商品分类列表']);
        Route::get('timed_deposit/amount', 'v1.store.CategoryController/timedDepositAmount')->name('timedDepositAmount')->option(['real_name' => '限时拍保证金金额']);
        Route::get('getscribemessage', 'v1.wechat.SendSubscribeMessageController/getScribeMessage')->name('getScribeMessage')->option(['real_name' => '查询订阅消息']);
        Route::get('sendsubscribemessage', 'v1.wechat.SendSubscribeMessageController/SendSubscribeMessage')->name('sendsubscribemessage')->option(['real_name' => '订阅消息']);
        Route::post('realNameAuth', 'v1.wechat.RealNameAuthController/recognize')->name('RealNameAuth')->option(['real_name' => '实名认证']);
        
       
       // Route::get('refund', 'v1.store.RefundController/refund')->name('Refund')->option(['real_name' => '实名认证']);
        //Route::get('refund/notify', 'v1.store.RefundController/notify')->name('notify')->option(['real_name' => '实名认证']);
        
        Route::get('category/auctionDetails', 'v1.store.CategoryController/auctionDetails')->name('auctionDetails')->option(['real_name' => '商品分类类']);
        Route::get('category/getproductDetails/:id', 'v1.store.CategoryController/getproductDetails')->name('getproductDetails')->option(['real_name' => '商品详情']);
        Route::get('agentPrice/detail/:product_id', 'v1.store.storeAgentPriceController/detail')->name('detail')->option(['real_name' => '取消代理出价']);
        Route::get('sendToSocket', 'v1.store.SendToSocketController/sendToSocket')->name('sendToSocket')->option(['real_name' => '取消代理出价']);
        //商品类
