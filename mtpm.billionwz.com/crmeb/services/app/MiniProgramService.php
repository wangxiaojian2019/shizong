        ...
        return self::paymentService()->handleNotify(function ($notify, $successful) {
            if ($successful && isset($notify->out_trade_no)) {
                if (isset($notify->attach) && $notify->attach) {
                    ...
                    if ($notify->attach === 'product_batch') {
                        return app()->make(PayNotifyServices::class)->wechatProductBatch($notify->out_trade_no, $notify->transaction_id ?? '', PayServices::WEIXIN_PAY);
                    }
                        ...
                        case 'funds_order_pay':  // 小程序支付管理的
                            ...
                            switch ($prefix) {
                                case 'cp':
                                    ...
                                    break;
                                case 'bp':
                                    // 合并支付单号，与订单支付 attach「product_batch」一致
                                    $data['attach'] = 'product_batch';
                                    break;
                                case 'hy':
                                    $data['attach'] = 'Member';
                                    break;
